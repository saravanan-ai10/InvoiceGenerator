...
export default function Dashboard() {
  ...
  const filteredInvoices = invoices.filter(inv => {
    ...
    const searchMatch = inv.invoice_number?.toLowerCase().includes(term) ||
    ...
    <div className="flex-1 overflow-y-auto p-8 bg-slate-50">
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-2">Manage your invoices, purchase orders, and quotations.</p>
        </div>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <TabsContent value="invoice" className="mt-0 outline-none">
          {/* We will render the same list for each tab but let's centralize the display logic */}
        </TabsContent>
        <TabsContent value="purchase_order" className="mt-0 outline-none" />
        <TabsContent value="quotation" className="mt-0 outline-none" />
      </Tabs>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="py-16 flex flex-col items-center justify-center text-slate-500 bg-slate-50/30">
              ...
              <p className="text-lg font-medium text-slate-700 mb-1">No documents found</p>
              <p className="text-sm">
                {searchTerm ? 'Try a different search term.' : `Create your first ${activeTab.replace('_', ' ')} to get started.`}
              </p>
            </div>
          ) : (
              ...
              <Table className="min-w-[800px]">
                <TableHeader className="bg-slate-50/80">
                  <TableRow className="border-slate-100">
                    <TableHead className="font-semibold text-slate-900 ml-4 pl-6">Document No.</TableHead>
